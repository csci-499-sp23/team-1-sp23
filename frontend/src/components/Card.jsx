...
class InfoCard extends Component {
  ...
  getWebResults = async () => {
    const response = await fetch(
      `https://www.googleapis.com/customsearch/v1/siterestrict?key=AIzaSyBlzvLIfshBsJsg97DoGkFO9olqi94AMEI&cx=b61ece1ccdeb442fd&q=${this.props.school.school_name}`
    );
    const data = await response.json();
    this.setState({
      webResults: data,
                  ...
                  <Box
                    sx={{
                      maxWidth: {
                        xs: "92vw",
                        sm: 350,
                        md: this.props.compareOpened ? 520 : 370,
                      },
                      display: "flex",
                      overflowX: "auto",
                      whiteSpace: "nowrap",
